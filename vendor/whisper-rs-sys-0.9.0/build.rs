...
fn main() {
    ...
    // Always re-copy so that patches to whisper.cpp/CMakeLists.txt are reflected.
    // cmake's incremental build handles avoiding unnecessary recompilation.
    let _ = std::fs::remove_dir_all(&whisper_root);
    ...
    // Invalidate cmake cache so the updated CMakeLists.txt is re-processed
    // (cmake tracks source changes but not policy or flag edits in cached vars).
    ...
    if cfg!(feature = "cuda") {
        ...
        // CUDA 12.8+ dropped compute_52/53 (Maxwell); CUDA 13.x dropped compute_70
        // (Volta). The whisper.cpp default of "52 61 70" fails. Target Turing (75)+.
        config.define("GGML_CUDA_ARCHITECTURES", "75;80;86;89;90");
        // Static CRT is enforced via cmake_policy(CMP0091) + CMAKE_MSVC_RUNTIME_LIBRARY
        // in whisper.cpp/CMakeLists.txt — see the patch at the top of that file.
    }
